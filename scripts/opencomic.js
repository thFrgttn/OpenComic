...
var compressedMime = {
  ...
  zip: ['application/zip', 'application/x-cbz', 'application/x-zip', 'application/x-zip-compressed'],
  rar: ['application/rar', 'application/x-cbr', 'application/x-rar', 'application/x-rar-compressed'],
  '7z': ['application/7z', 'application/x-cb7', 'application/x-7z', 'application/x-7z-compressed'],
  tar: ['application/tar', 'application/x-cbt', 'application/x-tar', 'application/x-tar-compressed'],
  pdf: ['application/pdf', 'application/x-bzpdf', 'application/x-gzpdf']
};

var compressedExtensions = {
  all: ['zip', 'cbz', 'rar', 'cbr', '7z', 'cb7', 'tar', 'cbt', 'pdf'],
  zip: ['zip', 'cbz'],
  rar: ['rar', 'cbr'],
  '7z': ['7z', 'cb7'],
  tar: ['tar', 'cbt'],
...
const storage = require(p.join(appDir, 'scripts/storage.js')),
  ...
  tagging = require(p.join(appDir, 'scripts/tagging/tagging.js'));
...
function startApp() {
  ...
  if (
    ...
    !inArray(electron.remote.process.argv[1], ['--no-sandbox', 'scripts/main.js', '.']) &&
    fs.existsSync(electron.remote.process.argv[1])
  )
    openComic(electron.remote.process.argv[1], false);
  else dom.loadIndexPage(false);
...
function mousePosition(e, mode) {
  if (mode == 'x') return e.originalEvent.touches ? e.originalEvent.touches[0].pageX : e.pageX ? e.pageX : e.clientX;
  else return e.originalEvent.touches ? e.originalEvent.touches[0].pageY : e.pageY ? e.pageY : e.clientY;
}
...
function escapeQuotes(str, mode = false) {
  if (mode === 'simples') return String(str).replace(/'/g, "\\'");
  else if (mode === 'doubles') return String(str).replace(/"/g, '\\"');
  else
    return String(str)
      .replace(/'/g, "\\'")
      .replace(/"/g, '\\"');
...
function zoomIn() {
  ...
  else if (factor >= 2) factor += 1;
  else if (factor >= 1.25) factor += 0.25;
  else if (factor >= 1.1) factor += 0.15;
  else factor += 0.1;
...
function generateAppMenu(force = false) {
  if (
    force ||
    generateAppMenuData.resetZoom !== electron.webFrame.getZoomFactor() ||
    generateAppMenuData.onReading !== onReading
  ) {
    ...
    var menuTemplate = [
      {
        ...
        submenu: [
          {
            label: language.menu.file.openFile,
            click: function() {
              openComicDialog();
            },
          ...
          {
            label: language.menu.file.openFolder,
            click: function() {
              openComicDialog(true);
            },
            accelerator: 'CmdOrCtrl+Shift+O'
          },
          {
            label: language.menu.file.addFile,
            click: function() {
              addComic();
            }
          },
          {
            label: language.menu.file.addFolder,
            click: function() {
              addComic(true);
            }
          },
      ...
      },
      {
        ...
        submenu: [
          {
            label: language.menu.view.resetZoom,
            enabled: electron.webFrame.getZoomFactor() != 1 ? true : false,
            click: function() {
              resetZoom();
              generateAppMenu();
            },
          ...
          {
            label: language.menu.view.zoomIn,
            click: function() {
              zoomIn();
              generateAppMenu();
            },
          ...
          {
            label: language.menu.view.zoomOut,
            click: function() {
              zoomOut();
              generateAppMenu();
            },
          ...
          { type: 'separator' },
          {
            label: language.menu.view.toggleFullScreen,
            click: function() {
      ...
      },
      {
        ...
        submenu: [
          {
            label: language.reading.firstPage,
            enabled: onReading,
            click: function() {
              reading.goStart();
            },
          ...
          {
            label: language.reading.previous,
            enabled: onReading,
            click: function() {
              reading.goPrevious();
            },
          ...
          {
            label: language.reading.lastPage,
            enabled: onReading,
            click: function() {
              reading.goEnd();
            },
      ...
      {
        label: language.menu.tagging.main,
        submenu: [
          {
            label: language.menu.tagging.openTagging,
            enabled: true,
            click: function() {
              tagging.openTagging();
            }
      ...
      },
      {
        ...
        submenu: [
          {
            label: language.menu.help.about,
            click: function() {
              showAboutWindow();
            }
          }
...
function showAboutWindow() {
  ...
  about.once('ready-to-show', function() {
    about.webContents
      .executeJavaScript(
        "document.querySelector('body').innerHTML = `" + template.load('about.body.html') + '`;',
        false
      )
      .then(function() {
        about.show();
...
hb.registerHelper('for', function(from, to, incr, options) {
  ...
  for (var i = from; i < to; i += incr) accum += options.fn(i);
...
hb.registerHelper('normalizeNumber', function(value, decimals) {
  ...
  if (num_d != 0) value = value + (/\./.test(value) ? '' : '.') + '0'.repeat(num_d - num_v);
...
function pathIsSupported(path) {
  if (
    inArray(mime.getType(path), compatibleMime) ||
    inArray(fileExtension(path), compressedExtensions.all) ||
...
function openComicDialog(folders = false) {
  if (folders) var properties = ['openDirectory'];
  else var properties = ['openFile'];
  ...
  dialog
    .showOpenDialog({
      ...
      filters: [{ name: language.global.comics, extensions: folders ? ['*'] : compatibleExtensions }]
    })
    .then(function(files) {
      if (files.filePaths && files.filePaths[0]) openComic(files.filePaths[0]);
    });
}
...
function openComic(filePath, animation = true) {
  if (pathIsSupported(filePath)) {
    ...
    if (mainPath === false) mainPath = path;

    if (onReading) reading.saveReadingProgress();

    if (selectImage) dom.openComic(animation, path, mainPath);
    else dom.loadIndexPage(animation, path, false, false, mainPath);
  }
}

function addComic(folders = false) {
  if (folders) var properties = ['openDirectory', 'multiSelections'];
  else var properties = ['openFile', 'multiSelections'];
...
function checkError(value, error = false) {
  if (value && typeof value.error != 'undefined') {
    if (error && value.error == error) return true;
